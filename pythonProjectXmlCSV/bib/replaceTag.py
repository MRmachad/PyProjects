...
class ReplaceTag:
    ...
    def verificaComum(self):
        ...
        tamanho_Real = len(self.tbTag)

        for idx1 in range(tamanho_Real):
            for idx2 in range(tamanho_Real):
    ...
    def verificaComum2(self):
        ...
        tamanho_real = len(self.auxfcpTag)
            ...
            with open(( str(self.rpComum) + "_" + xml[11: (xml.rfind("."))] + "_log.txt"), "a+") as log:
                ...
                for idx in range(len(self.tbTag)):
                    ...
                    if str(self.tbTag[idx]).lower() in textoXML.lower():
                        ...
                        apontamentoC = "{/Area1/Data1::[" + self.tbNode[idx] + "]" + self.tbAdress[idx] + "}"
                ...
                for idx in range(len(self.auxfcpTag)):
                    ...
                    if self.auxfcpTag[idx].lower() in textoXML.lower():
                        if (self.auxfcpTag[idx] +"_LS" and self.auxfcpTag[idx] +"_LI") in self.tbTag:                       #isto é uma valvula e tem apontamento

                            LS = self.tbAdress[self.tbTag.index(self.auxfcpTag[idx] +"_LS" )]
                            LI = self.tbAdress[self.tbTag.index(self.auxfcpTag[idx] +"_LI" )]
                            apontamentoC = self.auxfcpNode[idx] + ", " + self.auxfcpAdress[idx] + ", " + LS[(LS.find("[") + 1):LS.rfind("]")] + ", " + LI[(LI.find("[") + 1):LI.rfind("]")] + ", " + self.auxfcpTagName[idx]
